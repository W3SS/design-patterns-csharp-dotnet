- Real world example
	...
	and the preference for payments is chosen as A then B then C. 
	...
	If not, request will move forward to account B checking for amount if yes chain will be broken 
	otherwise the request will keep forwarding till it finds the suitable handler. 
